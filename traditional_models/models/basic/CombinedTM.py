...
class CombinedTM(nn.Module):  # 定义组合主题模型类
    def __init__(self, vocab_size, contextual_embed_size, num_topics=50, en_units=200, dropout=0.4):  # 初始化方法
        ...
        self.fc_contextual = nn.Linear(contextual_embed_size, vocab_size)  # 上下文嵌入到词汇表的全连接层
        # self.fc11 = nn.Linear(vocab_size + vocab_size, en_units)  # 注释掉的组合输入层
    ...
    def get_theta(self, x):  # 定义获取主题分布方法
        contextual = self.fc_contextual(x[:, self.vocab_size:])  # 提取上下文嵌入并转换
        # combined = torch.cat((x[:, :self.vocab_size], contextual), dim=1)  # 注释掉的组合特征
        combined = contextual  # 使用上下文特征

        mu, logvar = self.encode(combined)  # 编码获取均值和方差
    ...
    def forward(self, x):  # 定义前向传播方法
        ...
        loss = self.loss_function(x[:, :self.vocab_size], recon_x, mu, logvar)  # 计算损失（只使用词袋部分）
