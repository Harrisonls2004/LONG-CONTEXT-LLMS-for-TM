...
class DecTM(nn.Module):  # 定义DecTM模型类
    '''
        Discovering Topics in Long-tailed Corpora with Causal Intervention. ACL 2021 findings.
        基于因果干预的长尾语料库主题发现，ACL 2021 findings

        Xiaobao Wu, Chunping Li, Yishu Miao.  # 作者信息
    '''
    def __init__(self, vocab_size, num_topics=50, en_units=200, dropout=0.4):  # 初始化方法
        ...
        self.beta = nn.Parameter(nn.init.xavier_uniform_(torch.empty(num_topics, vocab_size)))  # 主题-词分布参数，Xavier初始化

    def get_beta(self):  # 定义获取主题-词分布方法
        return self.beta  # 返回主题-词分布参数
    ...
    def decode(self, theta):  # 定义解码方法
        norm_theta = F.normalize(theta, dim=1)  # 归一化主题分布
        norm_beta = F.normalize(self.beta, dim=0)  # 归一化主题-词分布
        d1 = F.softmax(self.decoder_bn(torch.matmul(norm_theta, norm_beta)), dim=1)  # 计算重构结果并应用softmax
        return d1  # 返回重构结果
